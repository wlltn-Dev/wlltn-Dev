<div align="center">
  <a href="https://git.io/typing-svg">
    <img 
      src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=22&pause=1000&color=F1F1F1&center=true&vCenter=true&random=false&width=700&lines=Bem-vindo+ao+meu+perfil!;Data+Analytics+%7C+Python+%7C+SQL;Future+Data+Scientist" 
      alt="Typing SVG"
    />
  </a>
</div>

#

### Sobre mim

Profissional formado em **Tecnologia em Logística** e graduando em **Engenharia de Software**.

Em transição estratégica para a **área de Dados**, com foco em **Python**, **SQL**, automação de processos e análise operacional.

Experiência prática em produtividade, rotinas operacionais e melhoria contínua, aplicando tecnologia para gerar eficiência e decisões orientadas por dados.

Construindo portfólio com projetos reais e evolução constante.

#

<h3 align="left">📫 Connect with me</h3>

[![E-mail](https://img.shields.io/badge/-Email-000?style=for-the-badge&logo=microsoft-outlook&logoColor=white)](mailto:wuell.souza@gmail.com)
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-000?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/wuelliton-souza-data/)
[![Instagram](https://img.shields.io/badge/-Instagram-000?style=for-the-badge&logo=instagram&logoColor=white)](https://instagram.com/souza_wlltn)

#

<h3 align="left">🚀 Tech Stack</h3>

<img align="left" alt="Python" title="Python" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/python/python-original.svg"/>
<img align="left" alt="SQL" title="SQL" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/mysql/mysql-original.svg"/>
<img align="left" alt="Pandas" title="Pandas" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/pandas/pandas-original.svg"/>
<img align="left" alt="JavaScript" title="JavaScript" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/javascript/javascript-original.svg"/>
<img align="left" alt="Git" title="Git" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/git/git-original.svg"/>
<img align="left" alt="GitHub" title="GitHub" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/github/github-original.svg"/>
<img align="left" alt="VS Code" title="VS Code" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/vscode/vscode-original.svg"/>
<img align="left" alt="Google Sheets" title="Google Sheets" width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/google/google-original.svg"/>

<br/>
<br/>

#

<h3 align="left">📊 GitHub Stats</h3>

<p>
  <img 
    align="left" 
    height="190"
    src="https://github-readme-stats.vercel.app/api?username=wlltn-Dev&show_icons=true&theme=tokyonight&locale=en&include_all_commits=true"
  />

  <img 
    align="left"
    height="190"
    src="https://github-readme-stats.vercel.app/api/top-langs/?username=wlltn-Dev&theme=tokyonight&layout=compact&langs_count=8"
  />
</p>

<br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/>

#

<h3 align="left"> Contributions</h3>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/wlltn-Dev/wlltn-Dev/output/github-contribution-grid-snake-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/wlltn-Dev/wlltn-Dev/output/github-contribution-grid-snake.svg">
  <img alt="snake animation" src="https://raw.githubusercontent.com/wlltn-Dev/wlltn-Dev/output/github-contribution-grid-snake.svg">
</picture>

#

## 📌 Featured Projects

🔹 Automação com Google Sheets + Apps Script  
🔹 Dashboard Operacional com Python  
🔹 Consultas SQL para análise de negócios  
🔹 Controle Financeiro Automatizado  

#

## 📚 Atualmente Estudando

- Python aplicado a Dados  
- SQL avançado  
- Engenharia de Software  
- Estrutura de Dados  
- Projetos práticos para portfólio  

#

## 🎯 Objetivos 2026

- Evoluir em Python e SQL  
- Criar projetos relevantes para portfólio  
- Conquistar oportunidade na área de Dados  ✔
- Crescer financeiramente e profissionalmente  
